%\documentclass[letter,10pt]{report}
\documentclass[10pt,twoside,openright,titlepage,letter]{book}
%\usepackage[hmargin={3.6cm}]{geometry}
%\usepackage[T1]{fontenc}
%\usepackage{lmodern} 
\usepackage{booktabs}
\usepackage{longtable}
\usepackage[english]{babel}
\usepackage{supertabular}
% Text styles
\newcommand\textstyleInternetlink[1]{\textcolor{blue}{#1}}
%\let\textquotedbl="
%\usepackage{xtab}
%\usepackage{multicol}
%\usepackage{fancyvrb}
%\usepackage{graphicx}
\usepackage{fancyhdr}
\usepackage{caption}
\usepackage{calc}
\usepackage{color}
\usepackage{graphicx}
\usepackage{wrapfig}
\usepackage{textcomp}
\usepackage[T1]{fontenc}
\usepackage{array}
\usepackage{hhline}

\DeclareGraphicsExtensions{.pdf,.png,.jpg}
\captionsetup[table]{aboveskip=5pt,belowskip=5pt,position=top,margin=10pt,font=small,labelfont=bf,format=hang}
\captionsetup[figure]{aboveskip=0pt,belowskip=0pt,position=top,margin=10pt,font=small,labelfont=bf,format=hang}
%\setlength{\textwidth}{6in}
%\setlength{\textheight}{9in}
%\setlength{\topmargin}{1in}
%\setlength{\oddsidemargin}{.75in}
%\setlength{\evensidemargin}{.75in}
\pagestyle{fancy}
\sloppy

\setcounter{secnumdepth}{2}

%% Use htlatex main.tex to convert to html
%%
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\includeonly{chap6}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Load Sphinx package for function reference chapter
% (Required by auto-generated reference.tex from Sphinx documentation)
% Must be loaded BEFORE hyperref to avoid link color conflicts
\usepackage{sphinx}

\usepackage{hyperref}
\hypersetup{pdftitle=PEBL Manual,
            pdfauthor=Shane T. Mueller,
            pdfpagemode=none,
            pdfstartview=FitH,
            pdfkeywords={PEBL, Psychology Building Experiment Language},
            colorlinks=true,
            linkcolor=black,
            citecolor=black,
            filecolor=black,
            urlcolor=black
}

\setlength{\parindent}{0mm}

%make grey box for headers
\definecolor{MyGray}{rgb}{0.7,0.7,0.7}
\makeatletter\newenvironment{graybox}{%
   \begin{lrbox}{\@tempboxa}\begin{minipage}{\columnwidth}}{\end{minipage}\end{lrbox}%
   \colorbox{MyGray}{\color{black}\usebox{\@tempboxa}}
}\makeatother
%make a new section header
\newcommand\sect[1]{{
\vspace{10pt}
\begin{graybox}
\hypersetup{hidelinks}
\Huge\color{black}\section{#1}
\end{graybox}
}}

\begin{document}

\pagenumbering{roman}
\rule{\textwidth}{1mm}
\begin{center}
\LARGE
\textbf{The PEBL Manual} \\

\large
Programming and Usage Guide for\\
The Psychology Experiment Building Language\\
PEBL Version 2.2\\
\vspace{0.7cm}\Large \textbf{Shane T.~Mueller}
\end{center}
\rule{\textwidth}{1mm}


\vspace{.4cm}

\normalsize
\noindent\copyright 2003-2025 Shane T.~Mueller \href{mailto:smueller@obereed.net}{smueller@obereed.net}\\
\href{mailto:shanem@mtu.edu}{shanem@mtu.edu}\\
\\
\\


Current for PEBL Version 2.2 -- Released 2025 \\
\href{http://pebl.sourceforge.net}{http://pebl.sourceforge.net}


\newpage
\normalsize
\noindent COPYRIGHT \copyright 2025 Shane T.~Mueller. All rights reserved.\\
\vspace{2cm}
ISBN  978-0-557-65817-6\\

\tableofcontents

\clearpage
\pagenumbering{arabic}
\renewcommand{\chaptermark}[1]{\markboth{\textbf{Chapter \thechapter}.\ \emph{#1}}{}}
\include{intro}
\include{chap3}
\include{chap4}
\include{chap5}
\include{layout}
\include{taskarchitecture}
\include{launcher}
\include{ui}
\include{testbattery}
\include{reference}
\include{colors}
\appendix

\end{document}
