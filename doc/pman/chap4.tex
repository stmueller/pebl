...
\subsection{Adaptive TextBoxes}

The \texttt{AdaptiveTextBox()} function creates a textbox that automatically adapts to fit content that doesn't fit in the specified dimensions. This is particularly useful for instructions or messages that need to be displayed on smaller screens (e.g., 800x600 minimum) where overflow could be a problem.

\textbf{Usage:}
\begin{verbatim}
   box <- AdaptiveTextBox(text, x, y, window, fontsize, 
                          width, height, adaptive, maxlines)
\end{verbatim}

\textbf{Parameters:}
\begin{itemize}
\item \texttt{text}: The text content to display
\item \texttt{x, y}: Position coordinates (upper-left corner)
\item \texttt{window}: Parent window object
\item \texttt{fontsize}: Initial font size in points
\item \texttt{width, height}: Target box dimensions in pixels
\item \texttt{adaptive}: Adaptation strategy - \texttt{0} (none), \texttt{"scalebox"}, or \texttt{"scalefont"}
\item \texttt{maxlines}: Maximum number of lines allowed (default: 30)
\end{itemize}

\textbf{Adaptation Strategies:}
\begin{itemize}
\item \texttt{0} -- No adaptation; creates standard textbox (text may overflow)
\item \texttt{"scalebox"} -- Expands box to fit all text maintaining aspect ratio, then scales down using zoom to original size. Preserves original font size and uses SDL2 anisotropic filtering for high-quality rendering.
\item \texttt{"scalefont"} -- Iteratively reduces font size until text fits within the specified dimensions. Preserves box dimensions exactly (minimum 8pt font size).
\end{itemize}

\textbf{Example:}
\begin{verbatim}
   ##Create long instruction text
   instructions <- "Welcome to the experiment. This is a " +
                   "long text that demonstrates adaptive " +
                   "textbox functionality..."
   
   ##Scale font to fit - reduces text size
   box1 <- AdaptiveTextBox(instructions, 50, 50, win, 
                           24, 400, 100, "scalefont")
   
   ##Scale box - preserves font size
   box2 <- AdaptiveTextBox(instructions, 50, 200, win, 
                           24, 400, 100, "scalebox")
\end{verbatim}

The function relies on the \texttt{textComplete} property (returns 1 if all text rendered, 0 if truncated) to detect when adaptation is complete.
...
PEBL can load both raw and compressed audio filen, including .wav, .mp3, .ogg, .flac, and .midi files, which are handled seamlessly with the \texttt{LoadSound()} function. This returns an audio stream object that can be played  with either the \texttt{PlayForeground()}  or \texttt{PlayBackground()} functions.  
The \texttt{PlayForeground()} function returns once the sound is finished playing; \texttt{PlayBackground()} returns immediately and the sound plays in a separate thread.  When using \texttt{PlayBackground}, playing can be stopped using the \texttt{Stop()} function.

The volume of an audio sample can be manipulated using \texttt{SetPanning}. Here, each channel (left and right) volume is set independently, from 0 to 1.0.  Also, the \texttt{SetPlayRepeats} will repeat playback a sound until stopped. This could be good for a signal that is played until a response is made.
...
\begin{itemize}
\item \texttt{OpenCOMPort(portnum,baud,mode)}
\item COMPORTSENDBYTE(port,message)
\item COMPORTGETBYTE(port)
\end{itemize}
...
to the file \texttt{stderr.txt}.  In addition, when a fatal error is signalled, a dialog box will appear that displays the main message, after which PEBL will shut down.
...
Sometimes, you want to exit a program at a specific point, but don't want the error message dialog box to appear.  You can use the \verb|ExitQuietly(<message>)|  function to do this.
...
\sect{Controlling the Video settings}
As part of PEBL 2.0, PEBL now uses hardware-accelerated graphics subsystems that permit better and more accurate control of the video system.  For the most part, this should operate transparently, but the aspects of the video system can be controlled both by command-line arguments and can be overridden within a script using special global variables.  The launcher works by using command-line arguments, but these can always be overridden within a script.  For the most part, these global variables must be set before a \texttt{MakeWindow()} function is called.  The video system has several distinct aspects: 

\subsection{Screen resolution}  By default, the window will open up the same size as the current window.  However, you can specify a specific resolution using the pulldown menu of the launcher, which also allows you to specify a custom screen size.  This is most useful if you are debugging or running on a multi-screen setup.  Regardless of what the specified screen resolution specified in the launcher, the selected screen resolution will be specified using the \verb|gVideoWidth| and \verb|gVideoHeight| global parameters, stores the specified values in pixels.
...
\subsection{Synchronize to vertical refresh signal (vsync)}
...
\verb|gScriptName| &Specifies the name on the window, and some printed output.\\
\verb|gVSync| &Determines whether script should be run with vsync on\\
