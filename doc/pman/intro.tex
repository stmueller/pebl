...
\texttt{yacc}) and the 2.0 branch of the SDL libraries (\texttt{SDL}, \texttt{SDL\_image},
...
basically unmaintained) sdl\_audioin library. As of 2.4, it supports web deployment with via emscripten (instance hosted at \url{https://peblhub.online}), labstreaminglayer for syncing data with external equipment, and use of 500+ psychological scales from the OpenScales project at \url{http://openscales.net}.
...
\sect{How to Compile PEBL 2.0}

Currently, there is no automated compile procedure.  PEBL requires the
\texttt{SDL2}, \texttt{SDL2\_image}, \texttt{SDL2\_gfx} \texttt{SDL2\_net},
 \texttt{SDL2\_audioin} and
\texttt{SDL2\_ttf} libraries and development headers.  It also uses
...
On Microsoft Windows, PEBL is designed to be compiled using minGW implementation of the gnu compiler toolset.

\subsection{MacOS}
As of version 2.4, we do not support MacOS directly, although we may in the future if there is demand. Users can use \url{https://peblhub.online} on OSX.


\sect{Installation}
\subsection{Linux}

On Linux, there is an .appimage package available.  However, it is fairly easy to 
...
invoke PEBL by typing `\texttt{pebl2}' at a command line.  PEBL requires you to
specify one or more source files that it will compile and run, e.g., the
command:
\begin{verbatim}
  > pebl2 stroop.pbl library.pbl
\end{verbatim}
...
\begin{verbatim}
  > pebl2.exe stroop.pbl library.pbl
\end{verbatim}
...
Examples:
\begin{verbatim}
pebl2 experiment.pbl -s sub1 --fullscreen --display 800x600
      --driver dga
pebl2 experiment.pbl --driver xf86  --language es
pebl2 experiment.pbl -v 33 -v 2 --fullscreen --display 640x480 
\end{verbatim}
...
--driver <drivername>
\end{verbatim}
  Sets the video driver, when there is more than one. On all platforms, opengl and software should
  be available.   In Linux, opengles is also available, and on windows, direct3d.
...
--language <code>
\end{verbatim}
  Specifies the language for translations. Sets the global variable gLanguage.
  Can be a two-letter ISO 639-1 code (e.g., ``en'', ``es'', ``fr'') or any
  custom identifier. The translation system uses a three-tier fallback:
  (1) try the specified language, (2) try the system locale if no language
  was specified, (3) fall back to English. If not set, defaults to empty
  string, triggering system locale detection.
...
--resizable
\end{verbatim}
  Controls whether the window will be resizeable. Only works in windowed mode.
...
--unicode
\end{verbatim}
  Turns on Unicode text handling, with slight performance overhead. Enabled by default.
  Required for displaying non-ASCII characters in multiple languages.

\item
\begin{verbatim}
--pfile <filename>
\end{verbatim}
  Specifies which parameter file to use. The filename is bound to the global variable
  gParamFile. Can be a local filename (prepended with ``params/'') or a full URL
  (http:// or https://) for remote parameter loading.

\item
\begin{verbatim}
--upload <filename>
\end{verbatim}
  Specifies an upload.json configuration file to use for syncing data with a
  remote data server (PEBL Simple Data Server or PEBLHub). Sets gUpload to true
  and gUploadFile to the specified filename.
...
--vsyncoff
\end{verbatim}
  Controls vertical synchronization (vsync). --vsyncon enables vsync for tasks
  requiring precise control of video refresh timing, but may be finicky on some
  hardware. --vsyncoff disables vsync (the default). The gVSync global variable
  is set accordingly and can be tested within scripts.

\item
\begin{verbatim}
--softrender
\end{verbatim}
  Forces use of software renderer instead of hardware-accelerated rendering.
  Automatically disables vsync. Sets the gSoftRender global variable to true.
  Useful for debugging graphics issues or running on systems without proper
  GPU drivers.
...
--help
\end{verbatim}
  Displays the command-line help screen with all available options.

\item
\begin{verbatim}
--showtestresults
\end{verbatim}
  Sets the global variable gShowTestResults to 1 (instead of 0). Allows a test
  to automatically show a screen with results at the end, useful for self-paced
  experiments or demonstrations.


\end{description}
\sect{System Status Output}
...
\subsubsection{Driver information}
...
Script name:                               [PEBL Launcher 2.0]
Mon Jun 20 22:02:24 2016
--------------------------------------------------------------------------------
Display Mode:  Width  (pixels)                    [1000]
Display Mode:  Height (pixels)                    [700]
...
Parameter file (gParamFile):                      [/usr/local/share/pebl2/pebl-lib/params/launcher.pbl.par]
Busy/Easy wait: (gSleepEasy):                     [1]
Executable name: (gExecutableName):               [pebl2]
Resource path: (gPEBLResourcePath):               [/usr/local/share/pebl2]
Resource path: (gPEBLBasePath):                   [/usr/local/share/pebl2]
Working directory: (gWorkingDirectory):           [/home/username/Documents/pebl-exp.2.0]
