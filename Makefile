...
## http=2 is curl
## CURL_NO_OLDIES: temporary flag to surface all deprecated curl API usage as errors
CXXFLAGS_LINUX =   -DPEBL_UNIX -DPEBL_LINUX -DENABLE_BINRELOC -DPREFIX=$(PREFIX) -DEXECNAME=$(EXECNAME) -DPEBLNAME=$(PEBLNAME) -DPEBLDIRNAME=$(PEBLDIRNAME) -DPEBL_VERSION=\"$(PEBL_VERSION)\" -DHTTP_LIB=2 -DCURL_NO_OLDIES
