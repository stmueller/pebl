...
\verb+Round+ &\verb+<num>+, \verb+<precision> (opt)+ &Rounds \verb+<num>+ to nearest integer, or optionally power of 1/ten precision.\\ 
...
\verb+SDTDPrime+&\verb+<hr>+,\verb+<far>+, \verb+<tol>+(opt) &Computes SDT dprime.\\
\verb+SDTBeta+&\verb+<hr>+,\verb+<far>+, \verb+<tol>+(opt)&Computes SDT beta.\\
...
\verb+CumSum+&\verb+<list-of-numbers>+&Returns the cumulative sums of a set of numbers\\
\verb+VecSum+&\verb+<list1>+,\verb+<list2>+&Returns the pairwise sums of two lists of numbers\\
\verb+VecTimes+&\verb+<list1>+,\verb+<list2>+&Returns the pairwise products of two lists of numbers\\
...
\verb+SegmentsIntersect+ & \verb+<ax,ay,+ \verb+bx,by,+ \verb+cx,cy,+ \verb+dx,dy>+  &Determines whether line segment ab intersects cd.\\
\verb+NonOverlapLayout+&\verb+<xmin,xmax,+ \verb+ymin,ymax,+ \verb+tol,num>+  &Creates a set of num points that don't overlap, but fails gracefully\\
\verb+LayoutGrid+& \verb+<minx,maxx,+ \verb+miny,maxy,+ \verb+height,width,+ \verb+vertical>+
...
\verb+CopyFile+     &\verb+<file>+,\verb|<newfile>|&Makes a copy of a file\\
...
\verb+WaitForNetwork+ \verb+   Connection+&  \verb+<port>+&Listens on a port until another computer connects, returning a network object\\
\verb+CloseNetwork+ \verb+   Connection+& \verb+<network>+& Closes network connection\\
...
\verb+CheckForNetwork+ \verb+   Connection+&\verb+<network>+&Checks for incoming connection\\
...
\verb+GetNumJoystickBalls+    &\verb+joystick_object+&Counts how many balls on a joystick\\
\verb+GetNum+ \verb+   JoystickButtons+  &\verb+joystick_object+&Counts how many buttons on a joystick\\
\verb+GetNumJoystickHats+     &\verb+joystick_object+&Counts how many hats on a joystick\\
\verb+GetJoystickAxisState+   &\verb+joystick_object+, \verb+axis_id+&Gets the state of a joystick axis\\
\verb+GetJoystickHatState+    &\verb+joystick_object+, \verb+hat_id+&Gets the state of a joystick hat\\
\verb+GetJoystick+ \verb+ButtonState+ &\verb+joystick_object+, \verb+button_id+&Gets the state of a joystick button\\
\verb+GetJoystickBallState+   &\verb+joystick_object+, \verb+ball_id+&Gets the state of a joystick ball\\
...
\verb+MakeWindow+ &\verb+<colorname>+ (opt),
                 \verb+<width>+(opt),
                  \verb+<height>+(opt) &Creates main window, in color named by argument, or grey if no argument is named\\ 
...
\verb|GetAbsolutePosition| & \verb|<widget>| & Gets the absolute window position of a widget\\
\verb+SetEditable+ &\verb+ <textbox>+ \verb+<status>+& Turns on or off the editing cursor\\ 
\verb+GetTextBoxCursor+ \verb+   FromClick+& \verb+<relx>, +\verb+<rely>+ & Gets a cursor position (in characters) from a mouse click.\\
...
\verb|GetLineBreaks|&\verb|<textbox>|&Computes the line breaks for a textbox text.\\

\verb|GetParent| & \verb|<widget>| & Gets the parent widget of a widget.\\
...
\verb+RemoveObjects+ &\verb+<list-of-objects>+ \verb+<parent>+ &Removes a (possibly nested) list of objects from a parent window\\ 
...
\verb+GetMouse+ \verb+   CursorPosition+ & &Gets [x,y] position of mouse\\
\verb+GetMouseState+&&Gets [x,y,b1,b2,b3] list of mouse state, including button states\\
\verb+SetMouse+ \verb+   CursorPosition+ &\verb+<x>+,\verb+<y>+ &Sets x,y position of mouse\\
...
\verb+MakeAttneave+&\verb+<radius>+, \verb+<numpoints>+, \verb+<minangle>+, \verb+<maxangle>+&Makes a complex ``Attneave'' polygon\\
...
\verb+SetPanning+ &\verb+<sound>+, +<left>+, +<right>+ &Sets volume of left and right channel.\\
\verb+SetPlayRepeats+ &\verb+<sound>+, +<reps>+ &Sets a repeat count on a sound playback.\\
...
\verb+LoadMovie+ &\verb+<movie_filename>+  \verb+<window>+, \verb+<width>+,\verb+<height>+ &Load a movie file\\
...
\verb+MakeScrollingTextBox+ &\verb+<text>+, \verb+<x>+, \verb+<y>+, \verb+<win>+, \verb+<fontsize>+, \verb+<width>+, \verb+<height>+, \verb+<linewrap>+ &Make a box for text that can be scrolled if too long.\\
...
\cmidrule(l{.5cm}r{.5cm}){1-3} 
\verb|VariableExists| & \verb|<variable-name>|&Tests whether a variable exists.\\
...
\verb+WaitForAnyKeyDown+ &\verb+<>+ &Waits until any key is detected in down state\\ 
...
\verb+WaitForAnyKeyDown+ \verb+   WithTimeout+ & \verb+<time>+ & Waits for a key to be pressed, but only for\verb+<time>+ ms\\ 
\verb+WaitForKeyListDown+ & \verb+<list-of-keyvals>+ & Waits until one of the keys is in down state\\ 
\verb+WaitForKeyPress+ &\verb+<key>+ &Waits until \verb+<key>+ is pressed\\ 
\verb+WaitForAnyKeyPress+ &\verb+<>+ &Waits until any key is pressed\\ 
\verb+WaitForKeyRelease+ &\verb+<key>+ & Waits until \verb+<key>+ is released\\ 
\verb+WaitForListKeyPress+ &\verb+<list-of-keys>+ &Waits until one of \verb+<list-of-keys>+ is pressed \\ 
\verb+WaitForListKeyPress+ \verb+WithTimeout+&\verb+<list-of-keyvals>+ \verb+<timeout>+ & Waits for either a key to be pressed or a time to pass.\\
...
\verb+WaitForMouseClick+ \verb+   WithTimeout+ &\verb+<timeout>+ &Waits until any of the mouse buttons is pressed, or a prespecified timout has elapsed. \\ 
\verb+WaitForClickOnTarget+ &\verb+<target>+ &Waits until any of a set of target objects are clicked.\\ 
\verb+WaitForClickOnTarget+ \verb+   WithTimeout+  &\verb+<target>+, \verb+<timeout>+ &Waits with a max time for a set of targets to be clicked.\\ 
...
\verb|ExitQuietly|&\verb|<message>|&Halts execution, printing a message to terminal but not a pop-up\\
...
\verb+CopyToClipboard+ & \verb+<text>+ & Puts argument in system clipboard.\\
\verb+CopyFromClipboard+ & \verb+<>+ &  Copies text from system clipboard.\\
\verb+GetVideoModes+&opt:\verb+<screen>+& Gets list of available screen resolutions\\
\verb+GetDrivers+&&Gets a list of possible video drivers\\
\verb+GetCurrent+ \verb+   ScreenResolution+&\verb+<>+& Gets the current widthxheight of the screen\\
...
\verb+GetEasyMultiChoice+&\verb+<text>+, \verb+<choices>+, &Simple select-multiple choice \\
                   &\verb+<output>+, \verb+<window>+, &\\
                   &\verb|<min>|(opt), \verb|<max>|(opt)&\\

\verb+CountDown+&\verb+<window>+, \verb+<bg>+(opt)&Displays a 3 2 1 countdown on screen\\
...
\verb+ShuffleWithout+ \verb+   Adjacents+ &\verb+<nested-list>+ & Shuffle specifying items that should not appear adjacently\\
...
\verb+SampleN+ \verb+WithReplacement+ &\verb+<list>+ \verb+<n>+& Returns a sublist of \verb+<n>+ items from a list \\ 
\verb+DesignLatinSquare+   &\verb+<list1>+ \verb+<list2>+ & Simple latin square\\ 
\verb+LatinSquare+ &\verb+<list>+& A simple latin square constructor\\ 
\verb+DesignGreco+ \verb+   LatinSquare+ &\verb+<list1>+ \verb+<list2>+ \verb+<list3>+ &Create design\\ 
\verb+DesignBalanced+ \verb+   Sampling+ &\verb+<list>+ \verb+<number>+ &Sample equally from list\\ 
\verb+DesignFull+ \verb+   Counterbalance+ &\verb+<list1>+ \verb+<list2>+ &Create all combinations\\ 
\verb+CrossFactorWith+ \verb+   outDuplicates+ &\hspace{1.8cm}\verb+<list>+ & Returns a list of all pairs of items in the list, excluding pairs that where an element appears twice. \\ 
& & \\ 
...
\verb+ConcatenateList+&\verb+<list>+, \verb|<sep>|(opt)& Combines list\\
\verb+ListToString+   &\verb+<list>+, \\
& \verb+<sep>+ (opt),\\
& \verb+<prebuffer>+(opt)& Concatenates all elements of a list into a single string\\
\verb+ModList+ & \verb|<list>|, \verb|<pre>|, \verb|<post>| & Adds pre- and post- elements to each list member\\
...
\verb+ReplaceChar+&\verb+<string>+ \verb+<char>+ \verb+<char2>+ & Substitutes  \verb+<char2>+ for \verb+<char>+ in \verb+<string>+.\\
\verb+SubstituteStrings+&\verb+<text>+,\verb+<replist>+& Makes multiple substitutions in text\\
